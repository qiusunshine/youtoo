...
public class MyRealtimeBlurView extends View {
    ...
    private float mBlurRadius, defRadius; // default 10dp (0 < r <= 25)
    ...
    public void setPlaceholderId(int placeholderId) {
        ...
        if (isDark()) {
            if (getContext() instanceof Activity) {
                ((Activity) getContext()).getWindow().setNavigationBarColor(Color.WHITE);
            }
            setBackground(getContext().getResources().getDrawable(com.example.hikerview.R.drawable.shape_half_trans_deep));
        } else if (mBlurRadius != 0) {
            setBackground(null);
        } else {
            setBackground(getContext().getResources().getDrawable(placeholderId));
        }
    }

    private int placeholderId = com.example.hikerview.R.drawable.shape_top_border;

    public MyRealtimeBlurView(Context context, AttributeSet attrs) {
        super(context, attrs);

        mBlurImpl = getBlurImpl(); // provide your own by override getBlurImpl()

        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.RealtimeBlurView);
        defRadius = a.getDimension(R.styleable.RealtimeBlurView_realtimeBlurRadius,
        ...
        public boolean onPreDraw() {
            ...
            if (decor != null && isShown() && prepare()) {
                ...
                if (!isDark()) {
                    ...
                    int[] mBackShadowColors = new int[]{0x00FFFFFF, 0xFFFFFFFF};
                    if (tempCanvas == null) {
                        tempCanvas = new Canvas(mBlurredBitmap);
                    } else {
                        tempCanvas.setBitmap(mBlurredBitmap);
                    }
                    ...
                    mBackShadowDrawableLR.setBounds(0, 0, mBlurredBitmap.getWidth(), mBlurredBitmap.getHeight());
                    mBackShadowDrawableLR.setStroke(0, 0);
                    mBackShadowDrawableLR.draw(tempCanvas);
    ...
    private boolean isDark() {
        ...
        if (nightModeFlags == Configuration.UI_MODE_NIGHT_YES) {
            if (PreferenceMgr.getBoolean(getContext(), "forceDark", true)) {
                return true;
            }
        }
        return false;
    ...
    public void setDefRadius(float defRadius) {
    ...
    protected BlurImpl getBlurImpl() {
        return new BlurImpl() {
            ...
            public void blur(Bitmap input, Bitmap output) {
                try {
                    mBlurredBitmap = FastBlur.blur(input, 25, true);
                } catch (Throwable e) {
                    e.printStackTrace();
                }
            }
