...
public class AdListActivity extends BaseSlideActivity {
    ...
    private void shareRules() {
        new XPopup.Builder(getContext())
                .asCenterList("请选择操作", new String[]{"分享到剪贴板", "分享为订阅文件"},
                        ((option, text) -> {
                            switch (text) {
                                case "分享到剪贴板":
                                    int size = rules.size();
    ...
    private void shareToFile() {
        StringBuilder builder = new StringBuilder("version:1\n");
        for (int i = 0; i < rules.size(); i++) {
            builder.append(rules.get(i).getDom()).append("::").append(rules.get(i).getRule());
            if (i != rules.size() - 1) {
                builder.append("\n");
            }
        }
        ...
        File ruleFile = new File(fileDirPath + File.separator + "share-ad-rules.txt");
        if (ruleFile.exists()) {
        ...
        try {
            FileUtil.stringToFile(builder.toString(), ruleFile.getAbsolutePath());
            ShareUtil.findChooserToSend(getContext(), "file://" + ruleFile.getAbsolutePath());
            ToastMgr.shortBottomCenter(getContext(), "正在分享" + rules.size() + "个拦截规则");
        } catch (IOException e) {
    ...
    private View.OnClickListener addBtnListener = v ->
            new XPopup.Builder(getContext()).asInputConfirm("新增广告拦截", "支持##分隔多条，最多同时导入1000条", null, null,
                    text -> {
