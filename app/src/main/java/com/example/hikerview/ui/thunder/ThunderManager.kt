...
//                    toast("磁力链接解析超时")
    ...
    private fun play(
        ...
        ThreadTool.runOnUI {
            ...
            val u: String? = XLTaskHelper.instance().getLoclUrl(savePath + File.separator + info.mFileName)
            if(u == null) {
                toast("当前文件格式不支持云播，试试第三方播放器吧")
                ShareUtil.findChooserToDeal(ActivityManager.instance.currentActivity, "file://" + savePath + File.separator + info.mFileName)
                return@runOnUI
            }
            consumer.consume(
                u!!,
