    ...
    private void showStores(View view) {
        ...
        if (jsPluginMsg < now) {
            ...
            new XPopup.Builder(getContext())
                    .asConfirm("使用提示", "以下插件库来自网络收集，插件库网站以及网站内的插件均不是本软件提供和维护，不要随意导入不清楚的插件", () -> {
    ...
    private JSListAdapter.OnItemClickListener onItemClickListener = new JSListAdapter.OnItemClickListener() {
        ...
        public void onLongClick(View view, int position) {
            ...
            String[] titles = rules.get(position).isEnable() ?
                    new String[]{"编辑插件", "Web编辑", "外部打开", "分享插件", "禁用插件", "禁用全部", "删除插件", "拖拽排序", "分享云剪贴板", "分享云剪贴板2", "分享云剪贴板5", "分享云剪贴板6", "修改加载时机"} :
                    new String[]{"编辑插件", "Web编辑", "外部打开", "分享插件", "启用插件", "启用全部", "删除插件", "拖拽排序", "分享云剪贴板", "分享云剪贴板2", "分享云剪贴板5", "分享云剪贴板6", "修改加载时机"};
...
//                                    Snackbar.make(recyclerView, "努力为您加载中，请稍候", Snackbar.LENGTH_SHORT).show();
                                ...
                                case "分享云剪贴板5":
                                    ...
                                    if (StringUtil.isNotEmpty(paste5)) {
                                        RuleImporterManager.share(RuleImporterManager.Importer.Num5, JSListActivity.this, paste5, rules.get(position).getName(), "插件名");
                                    }
                                    break;
                                case "分享云剪贴板6":
                                    String paste6 = getSharePaste(position);
                                    if (StringUtil.isNotEmpty(paste6)) {
                                        RuleImporterManager.share(RuleImporterManager.Importer.Num6, JSListActivity.this, paste6, rules.get(position).getName(), "插件名");
                                    }
                                    break;
                                case "修改加载时机":
