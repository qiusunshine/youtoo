    ...
    protected void onAttachedToWindow() {
        ...
        if (!thisView.isInEditMode()) {
            ...
            if (mRefreshContent == null) {
                ...
                super.addView(errorView, 0, new com.scwang.smartrefresh.layout.SmartRefreshLayout.LayoutParams(MATCH_PARENT, MATCH_PARENT));
        ...
        AnimatorListenerAdapter listener = new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (mRefreshListener != null) {
                    if(notify) {
                        mRefreshListener.onRefresh(com.example.hikerview.ui.view.SmartRefreshLayout.this);
                    }
                } else if (mOnMultiPurposeListener == null) {
                    finishRefresh(3000);
                }
                if (mRefreshHeader != null) {
                    mRefreshHeader.onStartAnimator(com.example.hikerview.ui.view.SmartRefreshLayout.this, mHeaderHeight,  (int) (mHeaderMaxDragRate * mHeaderHeight));
                }
                if (mOnMultiPurposeListener != null && mRefreshHeader instanceof RefreshHeader) {
                    if (notify) {
                        mOnMultiPurposeListener.onRefresh(com.example.hikerview.ui.view.SmartRefreshLayout.this);
                    }
...
//            reboundAnimator.setDuration(20000);
    ...
    protected void animSpinnerBounce(final float velocity) {
        if (reboundAnimator == null) {
            if (velocity > 0 && (mState == RefreshState.Refreshing || mState == RefreshState.TwoLevel)) {
                animationRunnable = new com.example.hikerview.ui.view.SmartRefreshLayout.BounceRunnable(velocity, mHeaderHeight);
                ...
                animationRunnable = new com.example.hikerview.ui.view.SmartRefreshLayout.BounceRunnable(velocity, -mFooterHeight);
            } else if (mSpinner == 0 && mEnableOverScrollBounce) {
                animationRunnable = new com.example.hikerview.ui.view.SmartRefreshLayout.BounceRunnable(velocity, 0);
            }
        }
    }
    ...
    protected void moveSpinnerInfinitely(float spinner) {
         ...
         *
         * 2.XML关闭
         *          <com.hiker.hikerview.ui.view.SmartRefreshLayout
        ...
        if (mEnableAutoLoadMore && !mFooterNoMoreData && isEnableRefreshOrLoadMore(mEnableLoadMore) && spinner < 0
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    if (mLoadMoreListener != null) {
                        mLoadMoreListener.onLoadMore(com.example.hikerview.ui.view.SmartRefreshLayout.this);
                    } else if (mOnMultiPurposeListener == null) {
                        finishLoadMore(2000);//如果没有任何加载监听器，两秒之后自动关闭
                    }
                    final OnLoadMoreListener listener = mOnMultiPurposeListener;
                    if (listener != null) {
                        listener.onLoadMore(com.example.hikerview.ui.view.SmartRefreshLayout.this);
                    }
                }
            }, mReboundDuration);
        }
...
//    }
    ...
    public ViewGroup.LayoutParams generateLayoutParams(AttributeSet attrs) {
        final View thisView = this;
        return new com.scwang.smartrefresh.layout.SmartRefreshLayout.LayoutParams(thisView.getContext(), attrs);
    }
...
//    }
    ...
    public RefreshLayout setRefreshHeader(@NonNull RefreshHeader header, int width, int height) {
        ...
        com.scwang.smartrefresh.layout.SmartRefreshLayout.LayoutParams lp = new com.scwang.smartrefresh.layout.SmartRefreshLayout.LayoutParams(width, height);
        Object olp = mRefreshHeader.getView().getLayoutParams();
        if (olp instanceof com.scwang.smartrefresh.layout.SmartRefreshLayout.LayoutParams) {
            lp = ((com.scwang.smartrefresh.layout.SmartRefreshLayout.LayoutParams) olp);
        }
...
//                      mKernel.setState(RefreshState.None);
                    ...
                    int startDelay = mRefreshHeader.onFinish(com.example.hikerview.ui.view.SmartRefreshLayout.this, success);
                    if (mOnMultiPurposeListener != null && mRefreshHeader instanceof RefreshHeader) {
                    ...
                    if (startDelay < Integer.MAX_VALUE) {
                        ...
                        if (mIsBeingDragged || mNestedInProgress) {
                            ...
                            if (mIsBeingDragged) {
                                ...
                                mTouchSpinner = 0;
                                mIsBeingDragged = false;
                                com.example.hikerview.ui.view.SmartRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX, mLastTouchY + mSpinner - mTouchSlop * 2, 0));
                                com.example.hikerview.ui.view.SmartRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mLastTouchY + mSpinner, 0));
                            }
                            if (mNestedInProgress) {
                                mTotalUnconsumed = 0;
                                com.example.hikerview.ui.view.SmartRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_UP, mLastTouchX, mLastTouchY, 0));
                                mNestedInProgress = false;
                                mTouchSpinner = 0;
                            }
                        }
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                if (count == 0) {
                    ...
                    final int startDelay = mRefreshFooter.onFinish(com.example.hikerview.ui.view.SmartRefreshLayout.this, success);
                    if (mOnMultiPurposeListener != null && mRefreshFooter instanceof RefreshFooter) {
                    ...
                    if (startDelay < Integer.MAX_VALUE) {
                        ...
                        if (mIsBeingDragged || mNestedInProgress) {
                            ...
                            if (mIsBeingDragged) {
                                ...
                                com.example.hikerview.ui.view.SmartRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX, mLastTouchY + offsetY + mTouchSlop * 2, 0));
                                com.example.hikerview.ui.view.SmartRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mLastTouchY + offsetY, 0));
                            }
                            if (mNestedInProgress) {
                                mTotalUnconsumed = 0;
                                com.example.hikerview.ui.view.SmartRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_UP, mLastTouchX, mLastTouchY, 0));
        ...
        if (mState == RefreshState.None && isEnableRefreshOrLoadMore(mEnableRefresh)) {
            Runnable runnable = new Runnable() {
                ...
                public void run() {
                    ...
                    final View thisView = com.example.hikerview.ui.view.SmartRefreshLayout.this;
                    mLastTouchX = thisView.getMeasuredWidth() / 2f;
                    mKernel.setState(RefreshState.PullDownToRefresh);
...
//    }
    ...
    public class RefreshKernelImpl implements RefreshKernel {
        ...
        public RefreshLayout getRefreshLayout() {
            return com.example.hikerview.ui.view.SmartRefreshLayout.this;
        }
...
//                            mRefreshFooter.getView().requestLayout();
        ...
        public ValueAnimator animSpinner(int endSpinner) {
            return com.example.hikerview.ui.view.SmartRefreshLayout.this.animSpinner(endSpinner, 0, mReboundInterpolator, mReboundDuration);
        }
        //</editor-fold>
