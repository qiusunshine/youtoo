...
public class AdUrlListActivity extends BaseSlideActivity {
    ...
    private void shareRules() {
        new XPopup.Builder(getContext())
                .asCenterList("请选择操作", new String[]{"分享最近10条规则", "分享最近20条规则", "分享最近50条规则", "分享最近100条规则", "分享最近1000条规则", "分享为订阅文件"},
                        ((option, text) -> {
                            switch (text) {
                                case "分享最近10条规则":
    ...
    private void shareToFile() {
        StringBuilder builder = new StringBuilder("version:2\n");
        for (int i = 0; i < rules.size(); i++) {
            builder.append(rules.get(i).getUrl());
            if (i != rules.size() - 1) {
                builder.append("\n");
            }
        }
        ...
        File ruleFile = new File(fileDirPath + File.separator + "share-ad-urls.txt");
        if (ruleFile.exists()) {
