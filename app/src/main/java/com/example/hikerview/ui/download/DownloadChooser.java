...
public class DownloadChooser {
    ...
    public static void startDownloadByThird(Activity context, String name, String url, @Nullable String originalUrl) {
        ...
        AlertDialog alertDialog = new AlertDialog.Builder(context).setTitle("选择下载器")
                .setSingleChoiceItems(names, 0, (dialog, which) -> {
                    dialog.dismiss();
                    startDownload(context, which + 1, name, finalUrl, originalUrl);
                }).setNegativeButton("取消", null).create();
        DialogUtil.INSTANCE.showAsCard(context, alertDialog);
    }

    public static boolean startDownload(Activity context, int downloader, String name, String url, @Nullable String originalUrl) {
        url = HttpParser.getThirdDownloadSource(DownloadChooser.getCanDownloadUrl(context, url));
        if (StringUtil.isEmpty(url)) {
            return false;
        }
        ...
        //非自带下载器
        if (downloader == 1) {
            return startSystemDownloader(context, name, url, originalUrl);
...
//        }
    ...
    private static boolean startSystemDownloader(Context context, String name, String url, @Nullable String originalUrl) {
        try {
