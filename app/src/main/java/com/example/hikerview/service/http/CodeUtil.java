...
//        JSEngine.getInstance().setUrl(url);
    ...
    public static void download(String url, String destFile0, Map<String, String> headers, OnCodeGetListener listener) {
        String destFile = JSEngine.getFilePath(destFile0);
    ...
    public static void downloadSync(String url, String destFile, Map<String, String> headers) throws Exception {
        if (url.startsWith("hiker://files/") || url.startsWith("file://")) {
            String path = JSEngine.getFilePath(url);
            if (!StringUtils.equals(path, destFile)) {
                FileUtil.copy(new File(path), new File(destFile));
        ...
        Response<byte[]> response = OkGo.<byte[]>get(url).headers(httpHeaders).converter(response1 -> {
            ResponseBody body = response1.body();
            byte[] bytes = body == null ? new byte[0] : body.bytes();
            try {
                if ("deflate".equals(response1.headers().get("Content-Encoding"))) {
                    //必须解码才能用
                    bytes = decompress(bytes);
                }
            } catch (Exception e) {
        ...
        FileUtil.bytesToFile(destFile, response.body());
    }

    public static Headers getHeaders(String url, Map<String, String> headers) {
        HttpHeaders httpHeaders = new HttpHeaders();
        if (headers != null && !headers.isEmpty()) {
            for (Map.Entry<String, String> entry : headers.entrySet()) {
                httpHeaders.put(entry.getKey(), entry.getValue());
        ...
        try {
            okhttp3.Response response = OkGo.<Headers>get(url).headers(httpHeaders).execute();
