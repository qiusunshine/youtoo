...
public class HikerRuleUtil {
    ...
    public static String getRulesByHiker(String url) {
        ...
        if (url.startsWith("home") || url.startsWith("rule") || url.startsWith("mini-program")) {
//            List<ArticleListRule> rules = LitePal.findAll(ArticleListRule.class);
            return JSON.toJSONString(MiniProgramRouter.INSTANCE.getData(), JSONPreFilter.getSimpleFilter());
    ...
    public static void getRulesByHiker(String url, CodeUtil.OnCodeGetListener listener) {
        try {
            ...
            } else if (url.startsWith("home") || url.startsWith("rule") || url.startsWith("mini-program")) {
                HeavyTaskUtil.executeNewTask(() -> {
//                    List<ArticleListRule> rules = LitePal.findAll(ArticleListRule.class);
                    listener.onSuccess(JSON.toJSONString(MiniProgramRouter.INSTANCE.getData(), JSONPreFilter.getSimpleFilter()));
                });
            } else if (url.startsWith("bookmark")) {
                HeavyTaskUtil.executeNewTask(() -> {
                    List<Bookmark> rules = LitePal.findAll(Bookmark.class);
