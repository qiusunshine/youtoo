    ...
    protected X5Extra generateExtra(String extra, String innerJs, String mode, String ticket) {
        ...
        if (StringUtil.isNotEmpty(extra)) {
            ...
            if (x5 != null) {
                ...
                if (StringUtil.isNotEmpty(x5.getUrlInterceptor())) {
                    x5Extra.setUrlInterceptor(x5.getUrlInterceptor());
                }
                x5Extra.setReferer(x5.getReferer());
                x5Extra.setJsLoadingInject(x5.isJsLoadingInject());
                if (StringUtil.isNotEmpty(x5.getJs())) {
                    extraJs = x5.getJs();
                }
                x5Extra.setRedirect(x5.isRedirect());
            }
        }
        x5Extra.setJs(generateExtraJS(innerJs, mode, ticket) + extraJs);
        return x5Extra;
    }
