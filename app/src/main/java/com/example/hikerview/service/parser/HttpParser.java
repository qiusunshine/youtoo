...
public class HttpParser {
    ...
    public static String parseSearchUrl(String sourceUrl, String wd) {
        return parseSearchUrl(sourceUrl, wd, false);
    }

    public static String parseSearchUrl(String sourceUrl, String wd, boolean fullUrl) {
        String[] d = sourceUrl.split(";");
        if (d.length == 1) {
            return replaceKey(sourceUrl, wd);
        } else if (d.length == 2) {
            if ("get".equalsIgnoreCase(d[1]) || "*".equals(d[1]) || "post".equalsIgnoreCase(d[1])) {

            } else {
                wd = encodeUrl(wd, d[1]);
            }
            return replaceKey(fullUrl ? sourceUrl : d[0], wd);
        } else {
            wd = encodeUrl(wd, d[2]);
            return replaceKey(fullUrl ? sourceUrl : d[0], wd);
        }
    ...
    public static String replaceKey(String url, String key) {
    ...
    private static String getRealUa(String uaCode) {
        ...
        for (UAEnum value : UAEnum.values()) {
            if (value.getCode().equals(uaCode)) {
                return value.getContent().replace(";", "；；");
            }
        }
        return uaCode.replace(";", "；；");
    }
    ...
    public static Map<String, String> getHeaders(String url, PlayData playData, int switchIndex) {
        Map<String, String> headers;
        if (CollectionUtil.isNotEmpty(playData.getHeaders()) && playData.getHeaders().size() > switchIndex) {
            headers = playData.getHeaders().get(switchIndex);
            if (headers != null && !headers.isEmpty()) {
                for (String s : headers.keySet()) {
                    headers.put(s, StringUtil.decodeConflictStr(headers.get(s)));
                }
            }
        } else {
