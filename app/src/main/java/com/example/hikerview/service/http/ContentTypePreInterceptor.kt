    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        try {
            ...
            if(userRequest.header("Cookie") != null){
                requestBuilder.header("Cookie-Temp", userRequest.header("Cookie")!!)
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }
        ...
        try {
            if(userRequest.url.host.contains("gitee.io") && userRequest.header("Referer") == null){
                requestBuilder.header("Referer", StringUtil.getHome(userRequest.url.toString()) + "/")
            }
