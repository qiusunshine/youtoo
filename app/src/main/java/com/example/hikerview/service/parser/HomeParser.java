...
public class HomeParser {
    ...
    public static void findList(String url, ArticleListRule movieChoose, String rule, String s, int page,boolean newLoad, BaseCallback<ArticleList> callback) {
        if (rule.startsWith("js:")) {
            Map<String, Object> injectMap = new HashMap<>();
            injectMap.put("MY_PAGE", page);
            JsEngineBridge.parseHomeCallBack(url, movieChoose.getCol_type(), movieChoose, rule, injectMap, s, newLoad, callback);
            return;
        }
