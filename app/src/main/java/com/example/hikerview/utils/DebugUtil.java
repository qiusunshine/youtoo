...
public class DebugUtil {
    ...
    public static void showErrorMsg(Activity activity, Context context, String title, String msg, String code, Throwable ex) {
        try {
            if ("规则执行过程中出现错误".equals(title) && StringUtil.isNotEmpty(msg) && msg.contains("\" is not defined.")) {
                //首页规则执行问题
                String[] strings = msg.split("\" is not defined");
                if (strings.length > 1) {
                    title = "软件版本过低或者规则有误";
                }
            }
            ...
            CustomBottomPopup popup = new CustomBottomPopup(activity, title).addOnClickListener(v -> {
                StringBuilder builder = new StringBuilder();
                builder.append("错误：").append(finalTitle);
                ...
                DialogUtil.INSTANCE.showAsCard(context, builder1.create());
            });
...
//                    }).show();
    ...
    public static void showErrorAllMsg(final Context context, final String msg) {
        try {
            ...
            AlertDialog alertDialog = new AlertDialog.Builder(context).setTitle("出现错误")
                    .setView(et)
                    .setCancelable(true)
                    .setPositiveButton("报告作者", (dialog, which) -> {
                        if (!ClipboardUtil.copyToClipboard(context, msg)) {
