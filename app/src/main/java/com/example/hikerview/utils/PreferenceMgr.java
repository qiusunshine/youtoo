...
public class PreferenceMgr {
    ...
    public static String getFilesDir(Context context) throws Throwable {
        Context c = context;
        int count = 5;
        while (count > 0) {
            count--;
            Map<String, Reflect> fields = Reflect.on(c).fields();
            if (fields.containsKey("mPreferencesDir")) {
                File file = fields.get("mPreferencesDir").get();
                return file.getAbsolutePath();
            } else if (fields.containsKey("mBase")) {
                c = fields.get("mBase").get();
            } else {
                break;
            }
        }
        throw new Exception("can not find mPreferencesDir");
    }
    ...
    public static float getFloat(Context context, String key, float defaultObject) {
        try {
            ...
            float real = getInt(context, FILE_NAME, key, (int) defaultObject) + 0f;
            put(context, FILE_NAME, key, real);
            return real;
        }
    }
    ...
    public static long getLong(Context context, String key, long defaultObject) {
        try {
            ...
            long real = (long) getInt(context, FILE_NAME, key, (int) defaultObject);
            put(context, FILE_NAME, key, real);
            return real;
        }
    }
