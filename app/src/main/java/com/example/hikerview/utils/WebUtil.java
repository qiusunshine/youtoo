...
public class WebUtil {
    ...
    public static Activity getWebViewActivity() {
        return Application.application.getHomeActivity();
    }
    ...
    public static void goWeb(Context context, String url) {
        if (context instanceof Activity && !(context instanceof WebViewActivity)) {
            ...
            if (size > 0) {
                for (int i = size - 1; i >= 0; i--) {
                    Activity activity = ActivityManager.getInstance().getActivityStack().elementAt(i);
        ...
        EventBus.getDefault().post(new OnUrlChangeEvent(url));
    }

    public static void goWebPage(Context context, String url) {
        MiniProgramRouter.INSTANCE.startWebPage(context, url, " ");
    }
